...
class SingleAddressResource(Resource):
    ...
    def get(self, address_id):
        addresses = Address.query.filter_by(user_id=address_id).all()
        if not addresses:
            return make_response(jsonify({"message": "Address not found"}), 404)

        addresses_dict = [address.to_dict(rules=("-user",)) for address in addresses]

        return make_response(jsonify(addresses_dict), 200)
...
api.add_resource(
    SingleAddressResource, "/addresses/<int:address_id>", endpoint="address"
)
